...
//			showToast(mBLUserID);
	...
	private View getHeaderView(ArrayList<PDetailGongChengEntity> bxInfo){
		...
		if(bxInfo!=null&&bxInfo.size()>0){
			...
			values.add(bxInfo.get(0).FFZongBaoHeTongE+"万元");
			values.add(bxInfo.get(0).FFYiShouKuanJinE+"万元");
			keys.add("项目编号");
			keys.add("项目名称");
			keys.add("总包合同额");
			keys.add("已收款金额");
			...
			for(int i=0;i<bxInfo.size();i++){
				...
				mList.add(bxInfo.get(i).FBJinE+"万元");
				mList.add(bxInfo.get(i).BenFenBaoHeTongYiZhiFu+"万元");
				mList.add(bxInfo.get(i).FFFaPiaoJinE+"万元");
